...
def get_object_name(object_type: int, object_color: int):
    object_types = {
        0: "EMPTY", 1: "FLOOR", 2: "WALL", 3: "BALL", 4: "SQUARE", 5: "PYRAMID",
        6: "GOAL", 7: "KEY", 8: "DOOR_LOCKED", 9: "DOOR_CLOSED", 10: "DOOR_OPEN",
        11: "HEX", 12: "STAR"
    }
    colors = {
        0: "EMPTY", 1: "red", 2: "green", 3: "blue", 4: "purple", 5: "yellow",
        6: "grey", 7: "black", 8: "orange", 9: "white", 10: "brown", 11: "pink"
    }
    if object_type == 0:
        return "EMPTY"
            ...
            try:
                ...
                def _chat():
                    return client.chat.completions.create(
                        model=openai_model,
                        messages=[
                            {"role": "system", "content": "You give concise, step-by-step hints for a gridworld."},
            ...
            try:
                ...
                def _claude():
                    return client.messages.create(
                        model=claude_model,
                        max_tokens=400,
                        messages=[{"role": "user", "content": f"{env_text}\n\nUser question:\n{prompt}"}],
                    )
...
async def env_stage_display_fn(stage: Stage, container: ui.element, timestep: TimeStep):
    ...
    with container.style("align-items: center;"):
        ...
        container.on('focusin', lambda e: asyncio.create_task(stage.set_user_data(active_container="env")))
        container.on('mousedown', lambda e: asyncio.create_task(stage.set_user_data(active_container="env")))

        # Header stats
        with ui.row():
            ui.label(f"Success: {stage_state.nsuccesses}/{stage.min_success}")
            ui.label().bind_text_from(stage_state, "nepisodes", lambda n: f"Try: {n}/{stage.max_episodes}")
        ...
        prompt_box.on('focus', lambda e: asyncio.create_task(stage.set_user_data(active_container="llm")))
        prompt_box.on('blur',  lambda e: asyncio.create_task(stage.set_user_data(active_container="env")))
...
async def feedback_display_fn(stage, container):
    ...
    with container.style("align-items: center;"):
        ...
        for q in questions:
            ui.label(q)
            ui.radio([1, 2, 3, 4, 5], on_change=lambda e, q=q: record_answer(q, e.value)).props("inline")
            answers[q] = None
        await done.wait()
        return answers
