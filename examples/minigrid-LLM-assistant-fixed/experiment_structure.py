...
class NavixTimestepWrapper(TimestepWrapper):
    ...
    def reset(self, key: jax.random.PRNGKey, params=None):
        ...
        resized = jax.image.resize(t.observation, shape=(128, 128, 3), method="bilinear").astype(jnp.uint8)
        return TimeStep(
    ...
    def step(self, key, state, action, params=None):
        ...
        return TimeStep(
            ...
            discount=jnp.ones((), dtype=jnp.float32),
            reward=t.reward,
            step_type=jnp.where(t.is_done(), jnp.array(2, dtype=jnp.uint8), jnp.array(1, dtype=jnp.uint8)),
        )
...
def _render_img(t: nicewebrl.TimeStep):
    return jax.image.resize(t.observation, shape=(256, 256, 3), method="nearest").astype(jnp.uint8)
...
async def env_stage_display_fn(stage: LLMEnvStage, container: ui.element, timestep: TimeStep):
