...
########################################
actions = [
  environment.Actions.right,
  environment.Actions.down,
  environment.Actions.left,
  environment.Actions.up,
]
...
jax_env = environment.Environment()
...
render_fn = jax.jit(environment.render_fn)

# precompile vmapped render fn that will vmap over all actions
vmap_render_fn = jax_web_env.precompile_vmap_render_fn(render_fn, example_env_params)
...
experiment = SimpleExperiment(stages=all_stages, name="Demo")
