...
class JaxWebEnv:
  def __init__(self, env, actions=None, render_fn: Optional[RenderFn] = None):
    ...
    if self.render_fn is not None:
      self.render_fn = jax.jit(self.render_fn)
...
class MultiAgentJaxWebEnv:
  def __init__(self, env, actions=None):
    ...
    def next_steps(rng, timestep, env_params, other_action=4, h_id=0):
      ...
      agent_0_action = jnp.where(
        h_id == 0, actions, jnp.repeat(other_action, actions.shape[0])
      )
      agent_1_action = jnp.where(
        h_id == 1, actions, jnp.repeat(other_action, actions.shape[0])
      )

      timesteps = jax.vmap(step_env, in_axes=(None, None, 0, 0, None), out_axes=0)(
        rng, timestep, agent_0_action, agent_1_action, env_params
      )
      return timesteps
  ...
  def precompile(
    self, dummy_env_params: struct.PyTreeNode = {"random_reset_fn": "reset_all"}
  ) -> None:
    """Call this function to pre-compile jax functions before experiment starts."""
  ...
  def precompile_vmap_render_fn(
    self,
    render_fn: RenderFn,
    dummy_env_params: struct.PyTreeNode = {"random_reset_fn": "reset_all"},
  ) -> RenderFn:
    """Call this function to pre-compile a multi-render function before experiment starts."""
